...
function initializeDB(callback){
	db = window.openDatabase("advancesas", "1.0", "Advance SAS", 1024*1024);
	callback();
}
...
function get63Questions(callback){
		
	 db.transaction(function(tx){ 
	 tx.executeSql("SELECT * FROM questions WHERE set_num != -1 AND subject = 'advance_sas'", [],
	 								function(tx, results){
										rows = results.rows;
										len = rows.length;
										...
										var first63Questions = new Array();
										for(i=0;i<63;i++){
											first63Questions.push(questions[i]);
										}
										callback(first63Questions);
									});	 	
...
function getPracticeQuestions(callback){
	 ...
	 db.transaction(function(tx){
	 					tx.executeSql("SELECT * FROM questions WHERE subject = 'advance_sas'", [],
	 								function(tx, results){
										rows = results.rows;
										len = rows.length;
										//alert(len);
